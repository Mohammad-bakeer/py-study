...
try:
    data = pandas.read_csv("flashcards/data/words_to_learn.csv")
except:
    original_data = pandas.read_csv("flashcards/data/french_words.csv")
...
def is_known():
    ...
    data.to_csv("flashcards/data/words_to_learn.csv", index=False)
    next_card()
...
back_image= PhotoImage(file="flashcards/images/card_back.png")
card_image = canvas.create_image(400,263, image=front_image)
card_titel = canvas.create_text(400,150,text="", font=("Ariel",40,"italic"))
card_word = canvas.create_text(400,263,text="", font=("Ariel",60,"bold"))
canvas.config(bg=BACKGROUND_COLOR, highlightthickness=0)
canvas.grid(row=0,column=0, columnspan=2)


cross_image = PhotoImage(file="flashcards/images/wrong.png")
